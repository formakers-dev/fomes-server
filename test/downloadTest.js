...
describe('Downloads', () => {
    it('다운로드 요청시 넘어온 Referer를 저장한다', done => {
        chai.request(server)
            .get('/download?referer=' + config.testAppbeeNumber)
            .send()
            .end((err, res) => {
                res.statusCode.should.be.eql(200);
                res.should.redirectTo('https://s3.ap-northeast-2.amazonaws.com/appbeepkg/release/appbee-beta.apk');

                DownloadHistories.find({'userId':config.testAppbeeNumber}, (err, histories) => {
    ...
    afterEach((done) => {
        DownloadHistories.remove({ userId : config.testAppbeeNumber }, () => {
            done();
        });
    });
});